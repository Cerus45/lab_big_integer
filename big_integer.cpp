...
std::vector<int> sub_abs(const std::vector<int>& a, const std::vector<int>& b) {
    ...
    for (size_t i = 0; i < a.size(); ++i) {
        long long diff = a[i] - borow - (i < b.size() ? b[i] : 0);
        if (diff < 0) {
            diff += BASE;
            borow = 1;
        } else {
...
BigInteger::BigInteger(const std::string& str) : negative_(false) {
    ...
    for (int i = static_cast<int>(str.length()); i > static_cast<int>(Starter); i -= 9) {
        int len = std::min(9, i - static_cast<int>(Starter));
        int Starter_idx = i - len;
        std::string chunk = str.substr(Starter_idx, len);
